...
dplyr::glimpse(clean_2000)
```

# 2003 data

```{r}
system(glue("Rscript {here('Code/2003/data_2003.R')}"))
...
```{r}
system(glue("Rscript {here('Code/2006/data_2006.R')}"))
...
dplyr::glimpse(clean_2006)
```

# 2009 data
```{r}
system(glue("Rscript {here('Code/2009/data_2009.R')}"))
...
clean_2009 = transform_pisa_variables(
  target_year = 2009, 
  df = stu_2009, 
  mapping_csv_path = here::here("variable_curation/PISA_variable_curation_student.csv")
)

skimr::skim(clean_2009)
dplyr::glimpse(clean_2009)
```
...
```{r}
system(glue("Rscript {here('Code/2012/data_2012.R')}"))
...
```{r}
system(glue("Rscript {here('Code/2015/data_2015.R')}"))
...
dplyr::glimpse(clean_2015)
```


# 2018 data
```{r}
system(glue("Rscript {here('Code/2018/data_2018.R')}"))
...
dplyr::glimpse(clean_2018)
```

# 2022 data
```{r}
system(glue("Rscript {here('Code/2022/data_2022.R')}"))
