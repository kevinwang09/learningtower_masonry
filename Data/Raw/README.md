...
It is important to note that the datasets for the year **2006/2009/2012** (found under the section "Data sets in TXT format" on the OECD website, which contains all the necessary `.zip` files) are protected by CloudFlare. Because of this security measure, they cannot be successfully retrieved via the automated download script and **must be manually downloaded** by developers into the `Data/Raw/yyyy` directories.
...
A system-generated central registry maintained and utilized by both scripts. It acts as the ultimate source of truth for the raw folder state.

**Schema Example:**

```json
{
    "2022/SCH_QQQ_SPSS.zip": {
        "year": "2022",
        "filename": "SCH_QQQ_SPSS.zip",
        "url": "https://webfs.oecd.org/pisa2022/SCH_QQQ_SPSS.zip",
        "md5sum": "db059ef65e4b6f67b361ce34f3053e01",
        "extracted_files": [
            "CY08MSP_SCH_QQQ.SAV"
        ]
    }
...
- `year`: The year of the data dataset.
- `filename`: The absolute filename.
- `url`: The URL that was used to download the file.
- `md5sum`: The computed MD5 checksum of the remote file.
- `extracted_files`: The exact list of extracted files contained within the ZIP archive (after extraction).

### `extracted_files_tree.json`

A lightweight JSON file generated by `prepare_raw_data.py` that summarizes the local folder structure and file sizes of the unzipped data.
...
- `type`: Indicates whether the item is a `"directory"` or a `"file"`.
- `name`: The name of the file or directory.
- `size_mb`: The size of the file in megabytes (only present for files).
- `children`: A list of nested objects representing the directory contents (only present for directories).
